...
```{r}
oil_consumption <- read_excel("../data/oil-consumption.xlsx")
 # change the class of the columns to numeric 
vec<- seq(2,57,1)
oil_consumption[ , vec] <- apply(oil_consumption[ , vec,drop=F], 2,           
...
oil_con <- oil_consumption %>% group_by(year) %>% summarise(Total_Consumption = sum(oil_comsumption_in_EJ)) %>% 
  select(Total_Consumption = Total_Consumption)
    # as_tsibble(index = year )
consumption_ts<-  ts(oil_con, start = 1965, frequency = 1)
